...
TYPE_NAME = "Terraform::MySQL::UserPassword"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["EncryptedPassword", "Id", "KeyFingerprint"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'mysql',
                'terraformTypeName': 'mysql_user_password',
            }).encode(),
        )
