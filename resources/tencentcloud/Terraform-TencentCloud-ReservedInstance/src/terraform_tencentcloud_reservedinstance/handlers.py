...
TYPE_NAME = "Terraform::TencentCloud::ReservedInstance"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["EndTime", "Id", "StartTime", "Status"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'tencentcloud_reserved_instance',
            }).encode(),
        )
