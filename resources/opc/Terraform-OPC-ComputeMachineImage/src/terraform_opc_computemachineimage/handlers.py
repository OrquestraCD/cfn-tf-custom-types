...
TYPE_NAME = "Terraform::OPC::ComputeMachineImage"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["ErrorReason", "Hypervisor", "Id", "ImageFormat", "NoUpload", "Platform", "State", "Uri"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'opc_compute_machine_image',
            }).encode(),
        )
