...
TYPE_NAME = "Terraform::OPC::ComputeInstance"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Attributes", "AvailabilityDomain", "Domain", "Entry", "Fingerprint", "Fqdn", "Id", "ImageFormat", "IpAddress", "PlacementRequirements", "Platform", "Priority", "QuotaReservation", "Relationships", "Resolvers", "Site", "StartTime", "State", "Vcable", "Virtio", "VncAddress"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'opc',
                'terraformTypeName': 'opc_compute_instance',
            }).encode(),
        )
