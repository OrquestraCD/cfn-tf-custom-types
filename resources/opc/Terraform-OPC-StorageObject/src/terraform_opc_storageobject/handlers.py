...
TYPE_NAME = "Terraform::OPC::StorageObject"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["AcceptRanges", "ContentLength", "Id", "LastModified", "ObjectManifest", "Timestamp", "TransactionId"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'opc_storage_object',
            }).encode(),
        )
