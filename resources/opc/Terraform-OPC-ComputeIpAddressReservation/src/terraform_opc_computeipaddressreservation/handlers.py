...
TYPE_NAME = "Terraform::OPC::ComputeIpAddressReservation"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "IpAddress", "Uri"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'opc_compute_ip_address_reservation',
            }).encode(),
        )
