...
TYPE_NAME = "Terraform::VSphere::DistributedVirtualSwitch"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["ConfigVersion", "Id"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vsphere_distributed_virtual_switch',
            }).encode(),
        )
