...
TYPE_NAME = "Terraform::VSphere::NasDatastore"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Accessible", "Capacity", "FreeSpace", "Id", "MaintenanceMode", "MultipleHostAccess", "ProtocolEndpoint", "UncommittedSpace", "Url"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vsphere_nas_datastore',
            }).encode(),
        )
