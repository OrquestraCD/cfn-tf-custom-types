...
TYPE_NAME = "Terraform::OpenTelekomCloud::MrsClusterV1"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["AvailableZoneName", "ChargingStartTime", "ClusterId", "ClusterState", "CoreNodeProductId", "CoreNodeSpecId", "CreateAt", "DeploymentId", "Duration", "ErrorInfo", "ExternalAlternateIp", "ExternalIp", "Fee", "HadoopVersion", "Id", "InstanceId", "InternalIp", "MasterNodeIp", "MasterNodeProductId", "MasterNodeSpecId", "OrderId", "PrivateIpFirst", "Remark", "SecurityGroupsId", "SlaveSecurityGroupsId", "TenantId", "UpdateAt", "Vnc"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'opentelekomcloud',
                'terraformTypeName': 'opentelekomcloud_mrs_cluster_v1',
            }).encode(),
        )
