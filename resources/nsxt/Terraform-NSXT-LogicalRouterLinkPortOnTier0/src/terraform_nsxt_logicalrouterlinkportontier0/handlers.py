...
TYPE_NAME = "Terraform::NSXT::LogicalRouterLinkPortOnTier0"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "LinkedLogicalRouterPortId", "Revision"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'nsxt',
                'terraformTypeName': 'nsxt_logical_router_link_port_on_tier0',
            }).encode(),
        )
