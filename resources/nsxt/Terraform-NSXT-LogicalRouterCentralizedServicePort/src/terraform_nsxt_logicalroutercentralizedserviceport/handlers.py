...
TYPE_NAME = "Terraform::NSXT::LogicalRouterCentralizedServicePort"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "Revision"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'nsxt_logical_router_centralized_service_port',
            }).encode(),
        )
