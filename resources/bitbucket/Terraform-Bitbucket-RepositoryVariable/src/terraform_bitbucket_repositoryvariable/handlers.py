...
TYPE_NAME = "Terraform::Bitbucket::RepositoryVariable"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "Uuid"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'bitbucket',
                'terraformTypeName': 'bitbucket_repository_variable',
            }).encode(),
        )
