...
TYPE_NAME = "Terraform::Brightbox::Server"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Fqdn", "Hostname", "Id", "Interface", "Ipv4Address", "Ipv4AddressPrivate", "Ipv6Address", "Ipv6Hostname", "Locked", "PublicHostname", "Status", "Username"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'brightbox',
                'terraformTypeName': 'brightbox_server',
            }).encode(),
        )
