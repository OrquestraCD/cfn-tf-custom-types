...
TYPE_NAME = "Terraform::Brightbox::DatabaseServer"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["AdminPassword", "AdminUsername", "Id", "Locked", "SnapshotsScheduleNextAt", "Status"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'brightbox_database_server',
            }).encode(),
        )
