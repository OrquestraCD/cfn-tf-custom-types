...
TYPE_NAME = "Terraform::Docker::Container"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Bridge", "ContainerLogs", "ExitCode", "Gateway", "Id", "IpAddress", "IpPrefixLength", "NetworkData"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'docker_container',
            }).encode(),
        )
