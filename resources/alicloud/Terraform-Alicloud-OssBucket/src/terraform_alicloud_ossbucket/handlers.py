...
TYPE_NAME = "Terraform::Alicloud::OssBucket"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["CreationDate", "ExtranetEndpoint", "Id", "IntranetEndpoint", "Location", "Owner"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_oss_bucket',
            }).encode(),
        )
