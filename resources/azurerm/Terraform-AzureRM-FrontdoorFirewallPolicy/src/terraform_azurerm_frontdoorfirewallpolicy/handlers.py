...
TYPE_NAME = "Terraform::AzureRM::FrontdoorFirewallPolicy"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["FrontendEndpointIds", "Id", "Location"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'azurerm_frontdoor_firewall_policy',
            }).encode(),
        )
