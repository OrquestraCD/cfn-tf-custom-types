...
TYPE_NAME = "Terraform::AzureRM::ApiManagement"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["GatewayRegionalUrl", "GatewayUrl", "Id", "ManagementApiUrl", "PortalUrl", "PublicIpAddresses", "ScmUrl"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'azurerm_api_management',
            }).encode(),
        )
