...
TYPE_NAME = "Terraform::AzureRM::NetworkInterface"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["AppliedDnsServers", "Id", "MacAddress", "PrivateIpAddress", "PrivateIpAddresses", "VirtualMachineId"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'azurerm_network_interface',
            }).encode(),
        )
