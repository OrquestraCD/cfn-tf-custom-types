...
TYPE_NAME = "Terraform::AzureRM::AppServiceCertificate"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["ExpirationDate", "FriendlyName", "HostNames", "Id", "IssueDate", "Issuer", "SubjectName", "Thumbprint"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'azurerm_app_service_certificate',
            }).encode(),
        )
