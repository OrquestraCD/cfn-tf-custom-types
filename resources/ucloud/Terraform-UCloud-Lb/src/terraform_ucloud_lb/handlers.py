            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["CreateTime", "ExpireTime", "Id", "IpSet", "PrivateIp"] and getattr(progress.resourceModel, return_value_name) is None:
