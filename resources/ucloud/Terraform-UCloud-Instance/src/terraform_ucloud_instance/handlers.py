...
TYPE_NAME = "Terraform::UCloud::Instance"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["AutoRenew", "Cpu", "CreateTime", "DiskSet", "ExpireTime", "Id", "IpSet", "Memory", "Status"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'ucloud',
                'terraformTypeName': 'ucloud_instance',
            }).encode(),
        )
