...
TYPE_NAME = "Terraform::OVH::DedicatedServerInstallTask"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Comment", "DoneDate", "Function", "Id", "LastUpdate", "StartDate", "Status"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'ovh_dedicated_server_install_task',
            }).encode(),
        )
