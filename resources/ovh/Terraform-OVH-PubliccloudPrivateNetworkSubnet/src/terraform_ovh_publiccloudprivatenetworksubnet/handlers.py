...
TYPE_NAME = "Terraform::OVH::PubliccloudPrivateNetworkSubnet"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Cidr", "GatewayIp", "Id", "IpPools"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'ovh_publiccloud_private_network_subnet',
            }).encode(),
        )
