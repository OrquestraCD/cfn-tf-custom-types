...
TYPE_NAME = "Terraform::Triton::Machine"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["ComputeNode", "Created", "Dataset", "Disk", "DomainNames", "Id", "Ips", "Memory", "Primaryip", "Type", "Updated"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'triton',
                'terraformTypeName': 'triton_machine',
            }).encode(),
        )
