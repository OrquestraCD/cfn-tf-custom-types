...
TYPE_NAME = "Terraform::Cloudflare::CustomSsl"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["ExpiresOn", "Hosts", "Id", "Issuer", "ModifiedOn", "Priority", "Signature", "Status", "UploadedOn"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'cloudflare_custom_ssl',
            }).encode(),
        )
