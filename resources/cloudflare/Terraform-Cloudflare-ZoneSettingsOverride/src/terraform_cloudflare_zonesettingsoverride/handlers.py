...
TYPE_NAME = "Terraform::Cloudflare::ZoneSettingsOverride"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "InitialSettings", "InitialSettingsReadAt", "ReadonlySettings", "ZoneStatus", "ZoneType"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'cloudflare_zone_settings_override',
            }).encode(),
        )
