...
TYPE_NAME = "Terraform::AzureAD::User"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "Mail", "ObjectId", "OnpremisesSamAccountName", "OnpremisesUserPrincipalName"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'azuread',
                'terraformTypeName': 'azuread_user',
            }).encode(),
        )
