...
TYPE_NAME = "Terraform::Scaleway::RdbInstanceBeta"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Certificate", "EndpointIp", "EndpointPort", "Id", "ReadReplicas"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'scaleway_rdb_instance_beta',
            }).encode(),
        )
