...
TYPE_NAME = "Terraform::Scaleway::K8sClusterBeta"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["ApiserverUrl", "CreatedAt", "Id", "Kubeconfig", "Status", "UpdatedAt", "UpgradeAvailable", "WildcardDns"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'scaleway_k8s_cluster_beta',
            }).encode(),
        )
