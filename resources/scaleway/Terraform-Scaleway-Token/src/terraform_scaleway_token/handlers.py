...
TYPE_NAME = "Terraform::Scaleway::Token"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["AccessKey", "CreationIp", "ExpirationDate", "Id", "SecretKey", "UserId"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'scaleway_token',
            }).encode(),
        )
