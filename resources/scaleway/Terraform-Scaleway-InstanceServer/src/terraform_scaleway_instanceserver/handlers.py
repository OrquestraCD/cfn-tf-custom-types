...
TYPE_NAME = "Terraform::Scaleway::InstanceServer"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["BootType", "Id", "Ipv6Address", "Ipv6Gateway", "Ipv6PrefixLength", "PlacementGroupPolicyRespected", "PrivateIp", "PublicIp"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'scaleway_instance_server',
            }).encode(),
        )
