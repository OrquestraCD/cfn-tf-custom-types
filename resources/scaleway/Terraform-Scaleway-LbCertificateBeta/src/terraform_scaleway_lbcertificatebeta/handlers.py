...
TYPE_NAME = "Terraform::Scaleway::LbCertificateBeta"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["CommonName", "Fingerprint", "Id", "NotValidAfter", "NotValidBefore", "Status", "SubjectAlternativeName"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'scaleway_lb_certificate_beta',
            }).encode(),
        )
