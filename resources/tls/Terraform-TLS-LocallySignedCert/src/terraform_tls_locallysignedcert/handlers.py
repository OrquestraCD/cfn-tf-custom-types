...
TYPE_NAME = "Terraform::TLS::LocallySignedCert"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["CertPem", "Id", "ReadyForRenewal", "ValidityEndTime", "ValidityStartTime"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'tls',
                'terraformTypeName': 'tls_locally_signed_cert',
            }).encode(),
        )
