...
TYPE_NAME = "Terraform::Nutanix::CategoryValue"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["ApiVersion", "Id", "SystemDefined"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'nutanix_category_value',
            }).encode(),
        )
