...
TYPE_NAME = "Terraform::Netlify::DeployKey"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "PublicKey"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'netlify',
                'terraformTypeName': 'netlify_deploy_key',
            }).encode(),
        )
