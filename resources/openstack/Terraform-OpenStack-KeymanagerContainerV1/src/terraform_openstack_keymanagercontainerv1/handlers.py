...
TYPE_NAME = "Terraform::OpenStack::KeymanagerContainerV1"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Consumers", "ContainerRef", "CreatedAt", "CreatorId", "Id", "Status", "UpdatedAt"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'openstack_keymanager_container_v1',
            }).encode(),
        )
