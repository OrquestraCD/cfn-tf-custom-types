...
TYPE_NAME = "Terraform::OpenStack::ObjectstorageObjectV1"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["ContentLength", "Date", "Id", "LastModified", "TransId"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'openstack_objectstorage_object_v1',
            }).encode(),
        )
