...
TYPE_NAME = "Terraform::AWS::NeptuneEventSubscription"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Arn", "CustomerAwsId", "Id"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_neptune_event_subscription',
            }).encode(),
        )
