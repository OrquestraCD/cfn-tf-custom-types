...
TYPE_NAME = "Terraform::AWS::DbInstance"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Address", "Arn", "Endpoint", "HostedZoneId", "Id", "Replicas", "ResourceId", "Status"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_db_instance',
            }).encode(),
        )
