...
TYPE_NAME = "Terraform::AWS::ElasticBeanstalkEnvironment"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["AllSettings", "Arn", "AutoscalingGroups", "Cname", "EndpointUrl", "Id", "Instances", "LaunchConfigurations", "LoadBalancers", "Queues", "Triggers"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_elastic_beanstalk_environment',
            }).encode(),
        )
