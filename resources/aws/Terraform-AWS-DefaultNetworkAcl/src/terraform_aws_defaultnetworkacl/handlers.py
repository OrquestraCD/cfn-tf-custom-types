...
TYPE_NAME = "Terraform::AWS::DefaultNetworkAcl"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        ...
                        if return_value_name in ["Id", "OwnerId", "VpcId"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_default_network_acl',
            }).encode(),
        )
