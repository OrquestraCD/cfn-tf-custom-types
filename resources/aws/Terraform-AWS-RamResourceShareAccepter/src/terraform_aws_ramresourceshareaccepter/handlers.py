...
TYPE_NAME = "Terraform::AWS::RamResourceShareAccepter"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "InvitationArn", "ReceiverAccountId", "Resources", "SenderAccountId", "ShareId", "ShareName", "Status"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_ram_resource_share_accepter',
            }).encode(),
        )
