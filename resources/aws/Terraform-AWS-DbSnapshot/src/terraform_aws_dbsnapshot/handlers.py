...
TYPE_NAME = "Terraform::AWS::DbSnapshot"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["AllocatedStorage", "AvailabilityZone", "DbSnapshotArn", "Encrypted", "Engine", "EngineVersion", "Id", "Iops", "KmsKeyId", "LicenseModel", "OptionGroupName", "Port", "SnapshotType", "SourceDbSnapshotIdentifier", "SourceRegion", "Status", "StorageType", "VpcId"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_db_snapshot',
            }).encode(),
        )
