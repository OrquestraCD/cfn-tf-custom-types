...
TYPE_NAME = "Terraform::AWS::KeyPair"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        ...
                        if return_value_name in ["Fingerprint", "Id", "KeyPairId"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_key_pair',
            }).encode(),
        )
