...
TYPE_NAME = "Terraform::AWS::CloudfrontDistribution"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["ActiveTrustedSigners", "Arn", "CallerReference", "DomainName", "Etag", "HostedZoneId", "Id", "InProgressValidationBatches", "LastModifiedTime", "Status"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_cloudfront_distribution',
            }).encode(),
        )
