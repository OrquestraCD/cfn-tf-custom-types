...
TYPE_NAME = "Terraform::AWS::DxHostedPrivateVirtualInterface"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["AmazonSideAsn", "Arn", "AwsDevice", "Id", "JumboFrameCapable"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_dx_hosted_private_virtual_interface',
            }).encode(),
        )
