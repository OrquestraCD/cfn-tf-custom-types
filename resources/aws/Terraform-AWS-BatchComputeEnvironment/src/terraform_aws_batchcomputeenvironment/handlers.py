...
TYPE_NAME = "Terraform::AWS::BatchComputeEnvironment"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Arn", "EccClusterArn", "EcsClusterArn", "Id", "Status", "StatusReason"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_batch_compute_environment',
            }).encode(),
        )
