            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Arn", "DefaultNetworkAclId", "DefaultRouteTableId", "DefaultSecurityGroupId", "DhcpOptionsId", "Id", "Ipv6AssociationId", "Ipv6CidrBlock", "MainRouteTableId", "OwnerId"] and getattr(progress.resourceModel, return_value_name) is None:
