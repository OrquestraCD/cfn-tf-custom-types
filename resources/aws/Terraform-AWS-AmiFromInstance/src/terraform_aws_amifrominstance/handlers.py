...
TYPE_NAME = "Terraform::AWS::AmiFromInstance"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Architecture", "EnaSupport", "Id", "ImageLocation", "KernelId", "ManageEbsSnapshots", "RamdiskId", "RootDeviceName", "RootSnapshotId", "SriovNetSupport", "VirtualizationType"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_ami_from_instance',
            }).encode(),
        )
