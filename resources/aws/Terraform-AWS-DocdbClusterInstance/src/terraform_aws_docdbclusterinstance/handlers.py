...
TYPE_NAME = "Terraform::AWS::DocdbClusterInstance"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Arn", "DbSubnetGroupName", "DbiResourceId", "Endpoint", "EngineVersion", "Id", "KmsKeyId", "Port", "PreferredBackupWindow", "PubliclyAccessible", "StorageEncrypted", "Writer"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_docdb_cluster_instance',
            }).encode(),
        )
