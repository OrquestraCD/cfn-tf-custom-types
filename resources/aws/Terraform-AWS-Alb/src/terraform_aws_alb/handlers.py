            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Arn", "ArnSuffix", "DnsName", "Id", "VpcId", "ZoneId"] and getattr(progress.resourceModel, return_value_name) is None:
