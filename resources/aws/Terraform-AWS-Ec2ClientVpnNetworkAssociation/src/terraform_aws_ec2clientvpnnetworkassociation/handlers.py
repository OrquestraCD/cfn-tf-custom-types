...
TYPE_NAME = "Terraform::AWS::Ec2ClientVpnNetworkAssociation"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "SecurityGroups", "Status", "VpcId"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_ec2_client_vpn_network_association',
            }).encode(),
        )
