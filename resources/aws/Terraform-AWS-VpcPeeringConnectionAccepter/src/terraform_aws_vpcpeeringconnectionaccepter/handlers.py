...
TYPE_NAME = "Terraform::AWS::VpcPeeringConnectionAccepter"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["AcceptStatus", "Id", "PeerOwnerId", "PeerRegion", "PeerVpcId", "VpcId"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_vpc_peering_connection_accepter',
            }).encode(),
        )
