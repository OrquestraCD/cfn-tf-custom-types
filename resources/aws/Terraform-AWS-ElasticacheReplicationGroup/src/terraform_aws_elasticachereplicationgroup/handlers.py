...
TYPE_NAME = "Terraform::AWS::ElasticacheReplicationGroup"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["ConfigurationEndpointAddress", "Id", "MemberClusters", "PrimaryEndpointAddress"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_elasticache_replication_group',
            }).encode(),
        )
