...
TYPE_NAME = "Terraform::AWS::EbsSnapshot"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["DataEncryptionKeyId", "Encrypted", "Id", "KmsKeyId", "OwnerAlias", "OwnerId", "VolumeSize"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_ebs_snapshot',
            }).encode(),
        )
