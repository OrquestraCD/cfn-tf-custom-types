...
TYPE_NAME = "Terraform::AWS::EbsSnapshotCopy"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["DataEncryptionKeyId", "Id", "OwnerAlias", "OwnerId", "VolumeId", "VolumeSize"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_ebs_snapshot_copy',
            }).encode(),
        )
