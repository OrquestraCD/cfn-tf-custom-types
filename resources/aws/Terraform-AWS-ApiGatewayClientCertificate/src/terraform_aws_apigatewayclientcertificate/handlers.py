...
TYPE_NAME = "Terraform::AWS::ApiGatewayClientCertificate"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Arn", "CreatedDate", "ExpirationDate", "Id", "PemEncodedCertificate"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_api_gateway_client_certificate',
            }).encode(),
        )
