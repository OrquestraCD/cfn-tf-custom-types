...
TYPE_NAME = "Terraform::PagerDuty::User"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["AvatarUrl", "HtmlUrl", "Id", "InvitationSent"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'pagerduty_user',
            }).encode(),
        )
