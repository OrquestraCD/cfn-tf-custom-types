...
TYPE_NAME = "Terraform::PagerDuty::Service"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["CreatedAt", "HtmlUrl", "Id", "LastIncidentTimestamp", "Status"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'pagerduty',
                'terraformTypeName': 'pagerduty_service',
            }).encode(),
        )
