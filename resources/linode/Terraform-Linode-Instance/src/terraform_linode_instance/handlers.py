...
TYPE_NAME = "Terraform::Linode::Instance"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Backups", "Id", "IpAddress", "Ipv4", "Ipv6", "PrivateIpAddress", "Specs", "Status"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'linode_instance',
            }).encode(),
        )
