...
TYPE_NAME = "Terraform::Packet::IpAttachment"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Address", "AddressFamily", "Cidr", "Gateway", "Global", "Id", "Manageable", "Management", "Netmask", "Network", "Public"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'packet_ip_attachment',
            }).encode(),
        )
