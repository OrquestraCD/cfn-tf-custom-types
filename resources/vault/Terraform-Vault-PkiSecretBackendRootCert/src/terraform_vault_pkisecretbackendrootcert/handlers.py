...
TYPE_NAME = "Terraform::Vault::PkiSecretBackendRootCert"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Certificate", "Id", "IssuingCa", "Serial"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vault_pki_secret_backend_root_cert',
            }).encode(),
        )
