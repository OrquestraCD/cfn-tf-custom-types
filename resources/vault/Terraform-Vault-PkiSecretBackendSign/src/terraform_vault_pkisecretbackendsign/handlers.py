...
TYPE_NAME = "Terraform::Vault::PkiSecretBackendSign"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["CaChain", "Certificate", "Expiration", "Id", "IssuingCa", "Serial"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vault_pki_secret_backend_sign',
            }).encode(),
        )
