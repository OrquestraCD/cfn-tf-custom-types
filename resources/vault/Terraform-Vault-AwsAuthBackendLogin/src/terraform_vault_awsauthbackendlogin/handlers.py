...
TYPE_NAME = "Terraform::Vault::AwsAuthBackendLogin"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Accessor", "AuthType", "ClientToken", "Id", "LeaseDuration", "LeaseStartTime", "Metadata", "Policies", "Renewable"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vault_aws_auth_backend_login',
            }).encode(),
        )
