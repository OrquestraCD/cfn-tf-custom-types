...
TYPE_NAME = "Terraform::Vault::TransitSecretBackendKey"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "Keys", "LatestVersion", "MinAvailableVersion", "SupportsDecryption", "SupportsDerivation", "SupportsEncryption", "SupportsSigning"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'vault',
                'terraformTypeName': 'vault_transit_secret_backend_key',
            }).encode(),
        )
