...
TYPE_NAME = "Terraform::Vault::ApproleAuthBackendRoleSecretId"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Accessor", "Id", "WrappingAccessor", "WrappingToken"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vault_approle_auth_backend_role_secret_id',
            }).encode(),
        )
