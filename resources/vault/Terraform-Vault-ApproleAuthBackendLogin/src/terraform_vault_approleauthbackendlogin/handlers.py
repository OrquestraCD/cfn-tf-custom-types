...
TYPE_NAME = "Terraform::Vault::ApproleAuthBackendLogin"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Accessor", "ClientToken", "Id", "LeaseDuration", "LeaseStarted", "Metadata", "Policies", "Renewable"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vault_approle_auth_backend_login',
            }).encode(),
        )
