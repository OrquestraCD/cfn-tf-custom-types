...
TYPE_NAME = "Terraform::VCD::IndependentDisk"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["DatastoreName", "Id", "Iops", "IsAttached", "OwnerName"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vcd_independent_disk',
            }).encode(),
        )
