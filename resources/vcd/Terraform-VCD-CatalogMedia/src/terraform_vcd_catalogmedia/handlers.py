...
TYPE_NAME = "Terraform::VCD::CatalogMedia"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["CreationDate", "Id", "IsIso", "IsPublished", "OwnerName", "Size", "Status", "StorageProfileName"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vcd_catalog_media',
            }).encode(),
        )
