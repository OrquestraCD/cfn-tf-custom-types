            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["AllocationIds", "Datacenters", "DeploymentId", "DeploymentStatus", "Id", "ModifyIndex", "Name", "Namespace", "Region", "TaskGroups", "Type"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'nomad',
                'terraformTypeName': 'nomad_job',
            }).encode(),
        )
