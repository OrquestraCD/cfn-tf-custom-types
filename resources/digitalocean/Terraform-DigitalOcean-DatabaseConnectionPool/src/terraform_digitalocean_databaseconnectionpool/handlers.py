...
TYPE_NAME = "Terraform::DigitalOcean::DatabaseConnectionPool"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Host", "Id", "Password", "Port", "PrivateHost", "PrivateUri", "Uri"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'digitalocean_database_connection_pool',
            }).encode(),
        )
