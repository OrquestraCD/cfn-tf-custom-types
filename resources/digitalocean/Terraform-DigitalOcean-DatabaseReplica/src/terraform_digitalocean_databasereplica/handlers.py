...
TYPE_NAME = "Terraform::DigitalOcean::DatabaseReplica"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Database", "Host", "Id", "Password", "Port", "PrivateHost", "PrivateUri", "Uri", "User"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'digitalocean_database_replica',
            }).encode(),
        )
