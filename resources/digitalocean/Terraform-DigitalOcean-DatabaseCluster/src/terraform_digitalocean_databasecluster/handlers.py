...
TYPE_NAME = "Terraform::DigitalOcean::DatabaseCluster"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Database", "Host", "Id", "Password", "Port", "PrivateHost", "PrivateUri", "Uri", "Urn", "User"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'digitalocean_database_cluster',
            }).encode(),
        )
