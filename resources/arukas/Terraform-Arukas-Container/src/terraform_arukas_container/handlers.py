...
TYPE_NAME = "Terraform::Arukas::Container"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["EndpointFullHostname", "EndpointFullUrl", "Id", "PortMappings", "Region", "ServiceId"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'arukas',
                'terraformTypeName': 'arukas_container',
            }).encode(),
        )
