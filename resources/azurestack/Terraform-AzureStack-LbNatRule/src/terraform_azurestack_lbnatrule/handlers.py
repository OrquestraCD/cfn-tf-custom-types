...
TYPE_NAME = "Terraform::AzureStack::LbNatRule"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["BackendIpConfigurationId", "FrontendIpConfigurationId", "Id"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'azurestack_lb_nat_rule',
            }).encode(),
        )
