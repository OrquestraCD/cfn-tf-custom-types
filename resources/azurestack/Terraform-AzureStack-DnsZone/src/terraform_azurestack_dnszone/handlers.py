...
TYPE_NAME = "Terraform::AzureStack::DnsZone"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "MaxNumberOfRecordSets", "NameServers", "NumberOfRecordSets"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'azurestack_dns_zone',
            }).encode(),
        )
