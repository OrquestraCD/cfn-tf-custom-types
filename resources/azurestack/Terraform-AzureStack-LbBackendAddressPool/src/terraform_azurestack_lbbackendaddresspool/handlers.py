...
TYPE_NAME = "Terraform::AzureStack::LbBackendAddressPool"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["BackendIpConfigurations", "Id", "LoadBalancingRules"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'azurestack_lb_backend_address_pool',
            }).encode(),
        )
