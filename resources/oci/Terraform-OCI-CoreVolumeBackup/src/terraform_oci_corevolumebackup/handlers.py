...
TYPE_NAME = "Terraform::OCI::CoreVolumeBackup"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["ExpirationTime", "Id", "KmsKeyId", "SizeInGbs", "SizeInMbs", "SourceType", "SourceVolumeBackupId", "State", "SystemTags", "TimeCreated", "TimeRequestReceived", "UniqueSizeInGbs", "UniqueSizeInMbs"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_core_volume_backup',
            }).encode(),
        )
