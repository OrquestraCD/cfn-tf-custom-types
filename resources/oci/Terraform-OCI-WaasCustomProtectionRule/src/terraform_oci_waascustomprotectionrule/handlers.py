...
TYPE_NAME = "Terraform::OCI::WaasCustomProtectionRule"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "ModSecurityRuleIds", "State", "TimeCreated"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_waas_custom_protection_rule',
            }).encode(),
        )
