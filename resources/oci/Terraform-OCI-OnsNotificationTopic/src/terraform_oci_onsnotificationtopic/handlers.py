...
TYPE_NAME = "Terraform::OCI::OnsNotificationTopic"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["ApiEndpoint", "Etag", "Id", "State", "TimeCreated", "TopicId"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_ons_notification_topic',
            }).encode(),
        )
