...
TYPE_NAME = "Terraform::OCI::CoreSubnet"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "Ipv6publicCidrBlock", "Ipv6virtualRouterIp", "State", "SubnetDomainName", "TimeCreated", "VirtualRouterIp", "VirtualRouterMac"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_core_subnet',
            }).encode(),
        )
