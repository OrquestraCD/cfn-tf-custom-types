...
TYPE_NAME = "Terraform::OCI::DatabaseAutonomousDatabase"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["ConnectionStrings", "ConnectionUrls", "Id", "IsPreview", "LifecycleDetails", "PrivateEndpoint", "ServiceConsoleUrl", "State", "SystemTags", "TimeCreated", "TimeDeletionOfFreeAutonomousDatabase", "TimeMaintenanceBegin", "TimeMaintenanceEnd", "TimeReclamationOfFreeAutonomousDatabase", "UsedDataStorageSizeInTbs"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_database_autonomous_database',
            }).encode(),
        )
