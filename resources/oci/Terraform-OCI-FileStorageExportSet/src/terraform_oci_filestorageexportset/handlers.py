...
TYPE_NAME = "Terraform::OCI::FileStorageExportSet"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["AvailabilityDomain", "CompartmentId", "Id", "State", "TimeCreated", "VcnId"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_file_storage_export_set',
            }).encode(),
        )
