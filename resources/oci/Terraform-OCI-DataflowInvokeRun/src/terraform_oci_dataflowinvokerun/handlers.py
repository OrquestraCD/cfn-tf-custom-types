...
TYPE_NAME = "Terraform::OCI::DataflowInvokeRun"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["ClassName", "DataReadInBytes", "DataWrittenInBytes", "FileUri", "Id", "Language", "LifecycleDetails", "OpcRequestId", "OwnerPrincipalId", "OwnerUserName", "RunDurationInMilliseconds", "SparkVersion", "State", "TimeCreated", "TimeUpdated", "TotalOcpu"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_dataflow_invoke_run',
            }).encode(),
        )
