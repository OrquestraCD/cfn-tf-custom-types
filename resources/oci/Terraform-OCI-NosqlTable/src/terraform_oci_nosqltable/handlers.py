...
TYPE_NAME = "Terraform::OCI::NosqlTable"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "LifecycleDetails", "Schema", "State", "TimeCreated", "TimeUpdated"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_nosql_table',
            }).encode(),
        )
