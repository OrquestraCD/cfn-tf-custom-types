...
TYPE_NAME = "Terraform::OCI::DatacatalogConnection"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["CreatedById", "ExternalKey", "Id", "Key", "State", "TimeCreated", "TimeStatusUpdated", "TimeUpdated", "UpdatedById", "Uri"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_datacatalog_connection',
            }).encode(),
        )
