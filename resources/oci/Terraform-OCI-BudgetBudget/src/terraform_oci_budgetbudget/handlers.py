...
TYPE_NAME = "Terraform::OCI::BudgetBudget"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["ActualSpend", "AlertRuleCount", "ForecastedSpend", "Id", "State", "TimeCreated", "TimeSpendComputed", "TimeUpdated", "Version"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_budget_budget',
            }).encode(),
        )
