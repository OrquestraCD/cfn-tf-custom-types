...
TYPE_NAME = "Terraform::OCI::DatabaseDbSystem"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "IormConfigCache", "LastPatchHistoryEntryId", "LifecycleDetails", "ListenerPort", "RecoStorageSizeInGb", "ScanDnsRecordId", "ScanIpIds", "State", "TimeCreated", "Version", "VipIds"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_database_db_system',
            }).encode(),
        )
