...
TYPE_NAME = "Terraform::OCI::CoreBootVolume"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "ImageId", "IsHydrated", "SizeInMbs", "State", "SystemTags", "TimeCreated", "VolumeGroupId"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_core_boot_volume',
            }).encode(),
        )
