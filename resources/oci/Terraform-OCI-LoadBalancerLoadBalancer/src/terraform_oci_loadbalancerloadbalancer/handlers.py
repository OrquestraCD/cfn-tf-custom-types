...
TYPE_NAME = "Terraform::OCI::LoadBalancerLoadBalancer"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "IpAddressDetails", "IpAddresses", "State", "SystemTags", "TimeCreated"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_load_balancer_load_balancer',
            }).encode(),
        )
