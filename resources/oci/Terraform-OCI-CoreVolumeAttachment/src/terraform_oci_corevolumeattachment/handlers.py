...
TYPE_NAME = "Terraform::OCI::CoreVolumeAttachment"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["AvailabilityDomain", "ChapSecret", "ChapUsername", "Id", "Ipv4", "Iqn", "Port", "State", "TimeCreated"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_core_volume_attachment',
            }).encode(),
        )
