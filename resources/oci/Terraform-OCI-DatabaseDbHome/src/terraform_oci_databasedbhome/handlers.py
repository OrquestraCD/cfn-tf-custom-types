...
TYPE_NAME = "Terraform::OCI::DatabaseDbHome"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["CompartmentId", "DbHomeLocation", "Id", "LastPatchHistoryEntryId", "LifecycleDetails", "State", "TimeCreated"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_database_db_home',
            }).encode(),
        )
