...
TYPE_NAME = "Terraform::OCI::OceOceInstance"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Guid", "Id", "IdcsTenancy", "Service", "State", "StateMessage", "TimeCreated", "TimeUpdated"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_oce_oce_instance',
            }).encode(),
        )
