...
TYPE_NAME = "Terraform::OCI::DatabaseAutonomousDatabaseInstanceWalletManagement"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "State", "TimeRotated"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_database_autonomous_database_instance_wallet_management',
            }).encode(),
        )
