...
TYPE_NAME = "Terraform::OCI::DatabaseDbNodeConsoleConnection"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["CompartmentId", "ConnectionString", "Fingerprint", "Id", "State"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_database_db_node_console_connection',
            }).encode(),
        )
