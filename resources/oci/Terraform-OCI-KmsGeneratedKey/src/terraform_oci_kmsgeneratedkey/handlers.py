...
TYPE_NAME = "Terraform::OCI::KmsGeneratedKey"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Ciphertext", "Id", "Plaintext", "PlaintextChecksum"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_kms_generated_key',
            }).encode(),
        )
