...
TYPE_NAME = "Terraform::OCI::OsmanagementManagedInstanceManagement"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["CompartmentId", "Description", "DisplayName", "Id", "LastBoot", "LastCheckin", "OsKernelVersion", "OsName", "OsVersion", "Status", "UpdatesAvailable"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_osmanagement_managed_instance_management',
            }).encode(),
        )
