...
TYPE_NAME = "Terraform::Google::AppEngineDomainMapping"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Id", "Name", "ResourceRecords"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'google_app_engine_domain_mapping',
            }).encode(),
        )
