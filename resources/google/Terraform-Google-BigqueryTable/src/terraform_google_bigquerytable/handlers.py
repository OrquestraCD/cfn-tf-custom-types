...
TYPE_NAME = "Terraform::Google::BigqueryTable"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["CreationTime", "Etag", "Id", "LastModifiedTime", "Location", "NumBytes", "NumLongTermBytes", "NumRows", "SelfLink", "Type"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'google_bigquery_table',
            }).encode(),
        )
