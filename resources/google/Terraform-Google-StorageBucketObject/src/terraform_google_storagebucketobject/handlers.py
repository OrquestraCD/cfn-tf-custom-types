...
TYPE_NAME = "Terraform::Google::StorageBucketObject"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Crc32c", "Id", "Md5hash", "OutputName", "SelfLink"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'google_storage_bucket_object',
            }).encode(),
        )
