...
TYPE_NAME = "Terraform::Google::ComputeImage"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["ArchiveSizeBytes", "CreationTimestamp", "Id", "LabelFingerprint", "SelfLink"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'google_compute_image',
            }).encode(),
        )
