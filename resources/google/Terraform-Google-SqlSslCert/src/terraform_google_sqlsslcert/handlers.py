...
TYPE_NAME = "Terraform::Google::SqlSslCert"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Cert", "CertSerialNumber", "CreateTime", "ExpirationTime", "Id", "PrivateKey", "ServerCaCert", "Sha1Fingerprint"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'google_sql_ssl_cert',
            }).encode(),
        )
