...
TYPE_NAME = "Terraform::Google::EndpointsService"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Apis", "ConfigId", "DnsAddress", "Endpoints", "Id"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'google_endpoints_service',
            }).encode(),
        )
