...
TYPE_NAME = "Terraform::Rundeck::PublicKey"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["Delete", "Id", "Url"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'rundeck',
                'terraformTypeName': 'rundeck_public_key',
            }).encode(),
        )
