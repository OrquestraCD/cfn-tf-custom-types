...
TYPE_NAME = "Terraform::NewRelic::Workload"
            ...
            try:
                if 'returnValues' in result:
                    for return_value_name, return_value_value in result['returnValues'].items():
                        if return_value_name in ["CompositeEntitySearchQuery", "Guid", "Id", "Permalink", "WorkloadId"] and getattr(progress.resourceModel, return_value_name) is None:
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerTypeName': 'newrelic',
                'terraformTypeName': 'newrelic_workload',
            }).encode(),
        )
